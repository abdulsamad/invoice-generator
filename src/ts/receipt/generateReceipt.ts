...
const generateInvoice = (target: string, formDataObj: any) => {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      addCompanyMobile(doc, COMPANY_CONTACT_NAME, COMPANY_CONTACT_MOBILE);
      ...
      addField(
        doc,
        {
          ...
          value: formDataObj["Received from Mr. / Ms."],
        },
        46
      );
      addField(
        doc,
        {
          ...
          value: formDataObj["Customer's Contact"],
        },
        53
      );
      addField(
        doc,
        { name: "Vehicle Name", value: formDataObj["Vehicle Name"] },
        60
      );
      addField(
        doc,
      ...
      // Download
      doc.save(
        `Receipt_${receiptNumber}_(${new Date().toLocaleDateString()}).pdf`
      );
      resolve("success");
    } catch (err) {
      reject(err);
